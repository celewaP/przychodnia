...
<body>
    ...
    <div class="menu">
        <ul>
            ...
            <li>
                <a href="admin.php" id="admin" style="display: block">Panel pracownika</a>
            </li>
            <li>
    ...
    <h1>KALENDARZ ZADAŃ</h1>
        <form method="post">
            ...
            <select name="miesiac">
                ...
                <input type="submit" id="wybierz" value="wybierz" name="wybierz"></select></div>
        </form>
        <div>
            <?php
            if(isset($_POST['wybierz'])){
            $miesiac=$_POST['miesiac'];
            $idlogowania=$_SESSION['idlogowania'];
            echo "<h1>".$miesiac."</h1>";
                $db=mysqli_connect("localhost", "root","","przychodnia");
                $sql="select kalendarz_pacjenta.data, kalendarz_pacjenta.opis, pracownicy.imie, pracownicy.nazwisko from kalendarz_pacjenta inner join pracownicy on kalendarz_pacjenta.idpracownika=pracownicy.idpracownika where miesiac='$miesiac' and idpacjenta='$idlogowania' order by kalendarz_pacjenta.data;";
                $query=mysqli_query($db,$sql);
                while($row=mysqli_fetch_array($query)){
                    echo "<div class='blok'>"."<h4>".$row['data']."</h4>";
                    echo "<p>".$row['opis']."</p>";
                    echo "<p style='text-decoration: underline;'>Dr ".$row['imie']." ".$row['nazwisko']."</p>"."</div>";
                }}
            ?>
        </div>
    </div>
